    ...
    namespace Mixing
    {
        ...
        class JJFemtoMixer
        {
            ...
            public:
                ...
                void SetEventHashingFunction(std::function<std::size_t(const Event &)> func) {fEventHashingFunction = std::move(func);}
                std::size_t GetEventHash(const Event &obj) const {return fEventHashingFunction(obj);}

                void SetPairHashingFunction(std::function<std::size_t(const Pair &)> func) {fPairHashingFunction = std::move(func);}
                std::size_t GetPairHash(const Pair &obj) const {return fPairHashingFunction(obj);}

                void SetPairCuttingFunction(std::function<bool(const Pair &)> func) {fPairCutFunction = std::move(func);}
                bool GetPairCutResult(const Pair &obj) const {return fPairCutFunction(obj);}

                void SetMaxBufferSize(const std::size_t &buffer) {fBufferSize = std::move(buffer);}
                std::size_t GetMaxBufferSize() const {return fBufferSize;}

                void SetFixedBuffer(const bool &isFixed) {fWaitForBuffer = isFixed;}
