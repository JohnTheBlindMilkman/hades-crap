...
void plotHBTscaling()
{
    ...
    // data from JJCorrFitter
    constexpr std::size_t centralitySize{4};
    constexpr unsigned long ktArrSize{7}; 
    float ktVal[] = {200,400,600,800,1000,1200,1400,1600};
    ...
    {{
        {3.00659,2.92792,2.40551,2.08228,1.77166,1.68787,1.71108},
    ...
    {{
        {0.311523,0.070727,0.044298,0.035440,0.035072,0.037443,0.047010},
    ...
    constexpr unsigned long yArrSize{4}; 
    float yVal[] = {0.16,0.39,0.62,0.86,1.09};
    constexpr std::array<std::array<float,yArrSize>,centralitySize> yRinvVal
    {{
        {1.49349,2.22013,2.65806,2.53566},
    ...
    {{
        {0.027869,0.031689,0.045167,0.070033},
    ...
    for (std::size_t cent = 0; cent < centralitySize; ++cent)
    {
        ...
        if (cent == 0)
        {
            hKtRinvArr[cent]->GetYaxis()->SetRangeUser(1.,4.);
            hKtRinvArr[cent]->Draw("p e1 x0");
        }
        else
        {
            hKtRinvArr[cent]->Draw("p e1 x0 same");
        }
    }
    ...
    for (std::size_t cent = 0; cent < centralitySize; ++cent)
    {
        TH1D *hYRinv = new TH1D(TString::Format("hYRinv_%ld",cent),";y_{c.m.} [a.u.];R_{inv} [fm]",yArrSize,yVal);
    ...
    for (std::size_t cent = 0; cent < centralitySize; ++cent)
    {
        ...
        if(cent == 0)
        {
            hYRinvArr[cent]->GetYaxis()->SetRangeUser(0.3,2.8);
            hYRinvArr[cent]->Draw("p e1 x0");
        }
        else
        {
            hYRinvArr[cent]->Draw("p e1 x0 same");
        }
    }
