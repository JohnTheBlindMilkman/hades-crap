...
int newFemtoAnalysis(TString inputlist = "", TString outfile = "femtoOutFile.root", Long64_t nDesEvents = -1, Int_t maxFiles = -1)
{
	...
	TCutG* betamom_1sig_p_tof_pionCmom = cutfile_betamom_pionCmom->Get<TCutG>("BetaCutProton_TOF_1.0");
	TCutG* betamom_1sig_p_rpc_pionCmom = cutfile_betamom_pionCmom->Get<TCutG>("BetaCutProton_RPC_1.0");
    ...
    for (Long64_t event = 0; event < nEvents; event++) 
    {
		...
		for (Int_t track = 0; track < nTracks; track++) 
		{
			...
			if (!fTrack->SelectTrack(betamom_1sig_p_rpc_pionCmom,betamom_1sig_p_tof_pionCmom))
				continue;
